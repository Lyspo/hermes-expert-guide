...
const BUDGETS = [
  ...
  },
  {
    ...
    //
    // Read this number for what it is. OGL plus the renderer are a lazy chunk fetched
    // when the canvas mounts — 14 kB gzipped, measured directly, which is the whole
    // argument for OGL over three.js at roughly 150 kB. This budget guards against OGL
    // *leaking into the initial payload*, which is the failure that would matter. It is
    // not a claim that the map is free.
    name: 'map (initial)',
    file: 'out/map/index.html',
    allowanceKb: 4,
  },
  {
    name: 'lesson',
