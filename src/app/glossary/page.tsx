  ...
  return (
    <Page>
      <main className="max-w-[64rem]">
        ...
        >
          {letters.map((letter) => (
            <a key={letter} href={`#letter-${letter}`} className="font-mono text-[0.8rem]">
              {letter}
            </a>
          ))}
        </nav>

        <dl className="mt-[calc(var(--step)*2.5)]">
          {glossary.map((entry, index) => (
            ...
            >
              {initial(entry.term) !== initial(glossary[index - 1]?.term ?? '') && (
                <h2
                  id={`letter-${initial(entry.term)}`}
                  className="scroll-mt-[calc(var(--step)*4)] mb-[calc(var(--step)*0.75)] font-mono text-[0.7rem] tracking-[0.12em] text-ice-dim uppercase"
                >
                  {initial(entry.term)}
                </h2>
              )}
              ...
              <dd className="mt-[calc(var(--step)*0.5)]">
                <p className="max-w-[68ch] text-[1.0625rem] leading-[1.7]">{entry.short}</p>
